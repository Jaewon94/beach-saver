...
| address                      | VARCHAR(255)  |     |     |                | 집하장 주소(임시)                                            |
| latitude                     | DECIMAL(10,7) |     |     |                | 위도                                                         |
| longitude                    | DECIMAL(10,7) |     |     |                | 경도                                                         |
| photo_url                    | VARCHAR(255)  |     |     |                | 집하장 사진(S3 등, 임시)                                     |
| status                       | VARCHAR(20)   |     |     |                | 상태(Enum: 수거대기/수거완료/문제 등, 임시)                  |
...
- collection_depot(임시 집하장)은 청소자가 청소 후 임의로 지정, 수거용 보고서(1:N) 하위 엔티티로만 관리
- 고정 집하장(행정상 공식 장소, collection_point)은 당분간 미사용/미구현(필요시 별도 도입)
